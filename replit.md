...
**Stamp Mechanic:** Bottom bar features physical 3D rubber stamp objects (wooden handle on top, colored rubber base on bottom) for APPROVE (green) and REJECT (red), plus a FREEZE (blue) stamp. Clicking a stamp triggers a pick-up animation (lifts upward with spring physics) and pick-up sound, then the stamp follows the cursor as a ghost preview. The stamp object on the bar fades to show it's "in hand." Bottom-right of the bar has a paper stack (stacked mini papers with slight rotation offsets, matching the bribe stack aesthetic). Clicking the paper stack pops up an "Inspector Decision Form" — a cream/manila paper card rising slightly above the bar with case info and a stamp target area. When a stamp is held, the target area shows "Stamp [Action] Here" — clicking it applies the stamp with an ink mark animation (mixBlendMode multiply, random rotation, distressed borders). The form auto-closes after 900ms. FREEZE auto-opens the form when clicked. Double-stamp prevention via stampLocked ref. Paper impact sound plays on stamp (reuses AudioContext via ref). Stamps and tools are mutually exclusive. Bribe logic preserved.
