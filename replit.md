...
**Stamp Mechanic:** Physical stamp tray replaces the old button bar at the bottom. APPROVE (green) and REJECT (red) stamps use cursor mode — click to select the stamp (background dims, "STAMP DOCUMENT" prompt appears), then click anywhere on the desk/documents to apply. FREEZE (blue escalation) is a direct action button. Stamps have distressed ink styling with SVG displacement filters, random rotation (-10° to +10°), uneven borders, and mix-blend-mode multiply for realistic ink-on-paper feel. Double-stamp prevention via stampLocked ref — input is locked from the moment of stamp application until the next client loads. Paper impact sound plays on stamp (reuses AudioContext via ref). Stamps and tools are mutually exclusive — selecting one clears the other. Bribe logic preserved.
