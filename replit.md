...
**Stamp Mechanic:** Bottom bar features large 3D rubber stamp objects (wooden handle on top, colored rubber base on bottom) for APPROVE (green, 72px wide), REJECT (red), and FREEZE (blue with snowflake). Clicking a stamp triggers a pick-up animation (lifts upward with spring physics) + pick-up sound, then a large ghost stamp (38px font) follows the cursor. The stamp fades to 30% on the bar. Bottom-right paper stack opens the stamp form. **Stamp form** pops up CENTERED on screen (z-700) with dark backdrop (z-600) — 380px wide, manila paper aesthetic. Shows case name, day, client number. Stamp target area shows "Pick up a stamp first" or "Click to stamp" when holding a stamp. Stamping produces a LARGE (52px) grunge ink mark with SVG displacement filter, random rotation (-10° to 10°), uneven border widths, mixBlendMode multiply, and scale-down impact animation (1.25x→1.0x). Heavy thud+slap stamp sound (80Hz sine + 200Hz sine + noise, 220ms). Form auto-closes after 900ms. FREEZE auto-opens form. Old decision feedback overlays (CitationModal, CorrectFlash, FraudEscalationModal) removed — stamp-on-card is the only feedback. Double-stamp prevention via stampLocked ref. Stamps and tools mutually exclusive. Bribe logic preserved.
